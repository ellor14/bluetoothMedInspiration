...
// opretter variabler
    ...
    @BindView(R.id.autoconnect)
            //skyde knappen Autocoonnect
    SwitchCompat autoConnectToggleSwitch;
    private RxBleDevice bleDevice;
    private Disposable connectionDisposable;

    // når det klikkes på knappen
    @OnClick(R.id.connect_toggle)
    public void onConnectToggleClick() {
        // vis den er connected lukkes forbindelse
        if (isConnected()) {
            triggerDisconnect();

            // vis den ikke er connected, opretter forbindelse
        } else {
    ...
    // når MTU knappen trykkes (MTU maximal transmisione unit
